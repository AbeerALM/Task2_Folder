...
dataset = pd.read_csv('/Users/abeer/Downloads/Crop_Production.csv')
...
output = open("/Users/abeer/downloads/ProductionOutput.txt", 'w')
#use discribe function to show the Value(float) statistics
output.write("Some Statistics about Production is Saudi Arabia based on the Item(plants)")
output.write("\n")
print(groupedf.describe(),file=output)
output.write("\n")
output.write("The variance for all Items(plants) Production is Saudi Arabia")
output.write("\n")
# Get Unbiased variance of the Value column for each group
var=X.groupby('Item')['Value'].var()
print(var,file=output)
output.write("\n")
output.write("The first row for each Items(plants) Production is Saudi Arabia with their Year Value")
output.write("\n")
...
for key, item in grouped:
    ...
    items.append(key)
with PdfPages(r'/Users/abeer/downloads/ProductionCharts.pdf') as export_pdf:
    for i in range(len(itemslist)):
        Y=[]
        y_values=[]
        x_values=[]
        output.write("The name of the item(value) Produced:  ")
        print(items[i],file=output)
        output.write("\n")
        output.write("The Table that shows the Year and Value of the item(value) Produced")
        output.write("\n")
    ...
    #draw the chart
        ...
        plt.ylabel('Production Value in (tonnes)',fontsize=8)
        plt.title(items[i]+' Production Value Vs Year')
        # create bar chart
        interactive(True)
        export_pdf.savefig()
        #plt.show()
...
#Please not if the graph is empty mean no production
    ...
    plt.xlabel('The mean Production Value in (tonnes)',fontsize=8)
    plt.title('The Items,plants in Saudi Arabia Vs Mean Production')
    interactive(False)
    export_pdf.savefig()
    #plt.show()
plt.close()
